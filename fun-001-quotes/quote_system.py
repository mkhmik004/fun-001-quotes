...
def read_file(file_name):
    try:
        with open(file_name,'r') as quotes:
            quotes=quotes.read()
...
def find_quote(random_quotee,quotes):
    if type(quotes[0])==list:
        quotes=quotes[0]
    for quote in quotes:
        quote_dict=quote.split('~')
        if quote_dict[0].strip().lower() in random_quotee.lower():
            return quote 
